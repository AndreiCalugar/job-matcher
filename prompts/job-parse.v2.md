...
Rules:
- `company_name`: the employer's proper name only, as written ("Pleo", "Danske Bank"). A description such as "a Series B fintech" or "our client" is not a name — use `null`.
...
- `required_skills`: concrete, named technologies, languages, frameworks, tools, and domains the posting requires. `importance` is `must` when phrased as required/essential/must-have, `should` when phrased as strong plus/ideally/preferred, `nice` when phrased as bonus/nice-to-have. List `nice` items in `required_skills` with importance `nice` AND repeat their names in `nice_to_have`. Keep names short and normalised ("Payments", not "Payments domain experience"). Soft skills ("team player") are not skills; omit them. Normalise names: "React.js" → "React", "Node" → "Node.js", "TS" → "TypeScript", "Postgres" → "PostgreSQL", ".NET Core" → ".NET", "k8s" → "Kubernetes".
